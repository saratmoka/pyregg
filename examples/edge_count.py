...
Edge Count — Example
====================
Estimate P(EC(G(X)) ≤ 15) for a Gilbert graph on [0,10]²
...
print("Edge Count: P(EC(G(X)) ≤ 15)")
print(f"  Window: [0,{wind_len}]²,  κ = {kappa},  r = {int_range}\n")

Z, RV, n = ec.naive_mc(wind_len, kappa, int_range, level, seed=42)
print(f"Naïve MC          Z = {Z:.4e}  RV = {RV:.2f}  n = {n:,}")

Z, RV, n = ec.conditional_mc(wind_len, kappa, int_range, level, seed=42)
print(f"Conditional MC    Z = {Z:.4e}  RV = {RV:.2f}  n = {n:,}")

Z, RV, n = ec.importance_sampling(wind_len, kappa, int_range, level, grid_res=100, seed=42)
