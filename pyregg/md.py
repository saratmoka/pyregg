...
Maximum Degree (MD)
===================
Rare-event estimation for the probability that the maximum degree of a
...
def naive_mc(wind_len, kappa, int_range, level,
    ...
    """
    Estimate P(MD(G(X)) ≤ level) using Naïve Monte Carlo.
    ...
    level : int
        Threshold ℓ. The rare event is {MD(G(X)) ≤ level}.
    ...
    -------
    probability : float
        Estimated rare-event probability P(MD(G(X)) ≤ level).
    rel_variance : float
    ...
    >>> Z, RV, n = md.naive_mc(wind_len=10, kappa=0.65, int_range=1.0, level=2)
    """
...
def conditional_mc(wind_len, kappa, int_range, level,
    ...
    """
    Estimate P(MD(G(X)) ≤ level) using Conditional Monte Carlo.
    ...
    --------
    >>> import pyregg.md as md
    >>> Z, RV, n = md.conditional_mc(wind_len=10, kappa=0.65, int_range=1.0, level=2)
    """
...
def importance_sampling(wind_len, kappa, int_range, level, grid_res=10,
    ...
    """
    Estimate P(MD(G(X)) ≤ level) using Importance Sampling Monte Carlo.

    Cells adjacent to any existing node with degree ≥ level, or with more
    than level existing neighbours, are blocked. A likelihood-ratio correction
    yields an unbiased estimate with substantially lower variance than CMC.
    ...
    --------
    >>> import pyregg.md as md
    >>> Z, RV, n = md.importance_sampling(wind_len=10, kappa=0.65, int_range=1.0,
    ...                                   level=2, grid_res=10)
    """
...
class _CallableModule(types.ModuleType):
    def __call__(self, wind_len, kappa, int_range, level, method="ismc", **kwargs):
        """
        Estimate P(MD(G(X)) ≤ level) using the specified method.
