...
==========================
Rare-event estimation for the probability that the number of triangles in a
...
def naive_mc(wind_len, kappa, int_range, level,
    ...
    """
    Estimate P(NTG(G(X)) ≤ level) using Naïve Monte Carlo.
    ...
    level : int
        Threshold ℓ. The rare event is {NTG(G(X)) ≤ level}.
    ...
    -------
    probability : float
        Estimated rare-event probability P(NTG(G(X)) ≤ level).
    rel_variance : float
    ...
    >>> Z, RV, n = ntg.naive_mc(wind_len=10, kappa=0.5, int_range=1.0, level=0)
    """
...
def conditional_mc(wind_len, kappa, int_range, level,
    ...
    """
    Estimate P(NTG(G(X)) ≤ level) using Conditional Monte Carlo.
    ...
    --------
    >>> import pyregg.ntg as ntg
    >>> Z, RV, n = ntg.conditional_mc(wind_len=10, kappa=0.5, int_range=1.0, level=0)
    """
...
def importance_sampling(wind_len, kappa, int_range, level, grid_res=10,
    ...
    """
    Estimate P(NTG(G(X)) ≤ level) using Importance Sampling Monte Carlo.

    Uses a two-phase blocking strategy: no blocking while the triangle count
    is below the threshold; once the threshold is reached, cells that would
    create a new triangle are blocked. A likelihood-ratio correction yields
    an unbiased estimate with substantially lower variance than CMC.
    ...
    --------
    >>> import pyregg.ntg as ntg
    >>> Z, RV, n = ntg.importance_sampling(wind_len=10, kappa=0.5, int_range=1.0,
    ...                                    level=0, grid_res=10)
    """
